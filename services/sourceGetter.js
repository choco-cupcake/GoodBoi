...
async function getAllSources(chain){ 
	...
	crawlInterval = setInterval(getSource, 300, chain)
...
function cleanVerificationDateHeader(source){ 
  if(source.substring(0,70).includes("Submitted for verification at ")){
    let index = source.indexOf("*/")
    if(index != -1){
      let ret = source.substring(index + 2)
      while(ret.substring(0,2) == "\r\n"){
        ret = ret.substring(2) // remove leading newlines
      }
      return ret
    }
  }
